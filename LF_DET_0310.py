...
class LF_DET_0310(nn.Module):
  def __init__(self, angRes_in, scale_factor, use_flashatten=False):
    ...
    if use_flashatten:
      ...
      torch.backends.cuda.enable_flash_sdp(enabled=True)  # : Enables or Disables FlashAttention.
      # torch.backends.cuda.enable_mem_efficient_sdp(enabled=True)  # : Enables or Disables Memory-Efficient Attention.
      # torch.backends.cuda.enable_math_sdp(enabled=True)  # : Enables or Disables the PyTorch C++ implementation.
  ...
  def forward(self, feature, angRes, patch_size):
    ...
    for num_windows in range(3):   # num_windows is M
      ...
      if (AH - ang_window_size) % ang_stride != 0:
        ...
        ang_features = self.ang_Transformer_Blocks[num_windows](rearrange(total_windows, 'b l c k1 k2 -> (b l) (k1 k2) c'))  # nopep8
...
if __name__ == "__main__":
  ...
  model = LF_DET_0310(5, 5, use_flashatten=True)
  model.eval()
  model.to('cuda:0')
  ...
  import tw
  print(tw.flops.summary(model, inp))
  # print(tw.flops.profile(model, inp, counts=5))
  ...
  # model_opt = LF_DET_C48(5, 5, use_flashatten=True)
  ...
  #   for i in range(10):
  #     t1 = time.time()
  #     out2 = model_opt(inp.to('cuda:0'))
  #     t2 = time.time()
  #     print(i, (t2 - t1) * 1000)

  # print((out2 - out1).sum(), (out2 - out1).mean())
